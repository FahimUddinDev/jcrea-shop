...
function LoginCardInner() {
  ...
  return (
    <div className="relative w-full max-w-md overflow-hidden rounded-3xl border border-slate-200 bg-white p-10 text-center shadow-xl shadow-slate-200/60">
      ...
      {/* Google Buttons: Admin & User */}
      <div className="mt-8 flex flex-col gap-3">
        {/* Sign in as Admin */}
        <button
          type="button"
          onClick={() => {
            document.cookie = "preferred_role=admin; path=/; max-age=300";
            signIn("google", { callbackUrl });
          }}
          className="flex w-full items-center justify-center gap-3 rounded-xl border border-orange-200 bg-orange-50/80 px-4 py-3 text-sm font-semibold text-orange-900 shadow-sm transition hover:border-orange-300 hover:bg-orange-100 active:scale-[0.98]"
        >
          ...
          <span>
            Sign in as{" "}
            <strong className="font-extrabold text-orange-600">Admin</strong>
          </span>
        </button>
        ...
        <button
          type="button"
          onClick={() => {
            document.cookie = "preferred_role=user; path=/; max-age=300";
            signIn("google", { callbackUrl });
          }}
          className="flex w-full items-center justify-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 hover:bg-slate-50 active:scale-[0.98]"
        >
          <svg viewBox="0 0 24 24" className="h-5 w-5 flex-shrink-0">
            <path
              fill="#4285F4"
